...
void sl_byte_convert__to_network_from_uint16(uint8_t bytes[2],
...
int sl_utils__pointer_distance(const void *pointer_to_last,
                               const void *string) {
  return (pointer_to_last - string);
}

static sl_string_size_t
sl_string__private_return_positive_or_zero(const int value) {
  return (value > 0) ? (sl_string_size_t)value : 0U;
}

static bool sl_string__private_ensure_enough_memory_for_insertion(
    const sl_string_t string, const char *insertion_string) {
 ...
 */
static char *sl_string__private_strtok_r(sl_string_t string,
...
sl_string_t sl_string__initialize_from(void *memory,
                                       sl_string_size_t memory_size_max,
...
void sl_string__clear_all_memory(sl_string_t string) {
  memset(string, 0, sl_string__get_capacity(string));
}
...
sl_string_size_t sl_string__printf(sl_string_t string, const char *format,
  ...
  const sl_string_size_t chars_printed =
...
sl_string_size_t sl_string__printf_append(sl_string_t string,
                                          const char *format, ...) {
  va_list args;
  va_start(args, format);
  const sl_string_size_t chars_printed =
      sl_string__vprintf_append(string, format, args);
  va_end(args);

  return chars_printed;
}

sl_string_size_t sl_string__vprintf_append(sl_string_t string,
                                           const char *format, va_list args) {
  const sl_string_size_t existing_length = sl_string__get_length(string);
  const sl_string_size_t remaining_capacity =
      sl_string__get_capacity_including_null_char(string) - existing_length;
...
int sl_string__tokenize(sl_string_t string, const char *delimiter_string,
                        int char_ptr_count, ...) {
  ...
  if (NULL != delimiter_string) {
    ...
    char *token =
        sl_string__private_strtok_r(string, delimiter_string, &save_pointer);
    ...
    while ((NULL != token) && (char_ptr_count > 0)) {
      ...
      token =
          sl_string__private_strtok_r(NULL, delimiter_string, &save_pointer);
...
int sl_string__last_index_of(const sl_string_t string, const char *index_of) {
  ...
  if (NULL != index_of) {
    ...
    while (NULL != pointer_to_first_in_substring) {
      ...
      pointer_to_first_in_substring =
          strstr(pointer_to_first_in_substring + substring_length, index_of);
    }
  }

  return ((NULL != pointer_to_last)
              ? (int)sl_utils__pointer_distance(pointer_to_last, string)
              : sl_string_error);
}

int sl_string__last_index_of_ignore_case(const sl_string_t string,
                                         const char *index_of_case_ignored) {
  int index_of_string = sl_string_error;

  if (NULL != index_of_case_ignored) {
    const char *pointer_to_first_char_in_substring =
        strchr(string, tolower((int)(*index_of_case_ignored)));
    if (NULL == pointer_to_first_char_in_substring) {
      pointer_to_first_char_in_substring =
          strchr(string, toupper((int)(*index_of_case_ignored)));
    }
    ...
    while (NULL != pointer_to_first_char_in_substring) {
      ...
      if (0 == strncasecmp(pointer_to_first_char_in_substring,
                           index_of_case_ignored, length_of_string_to_find)) {
        index_of_string = (int)sl_utils__pointer_distance(
            pointer_to_first_char_in_substring, string);
        num_of_chars_to_move_substring_start_location =
            length_of_string_to_find;
      }

      const char *pointer_to_last_char = pointer_to_first_char_in_substring;
      pointer_to_first_char_in_substring = strchr(
          pointer_to_last_char + num_of_chars_to_move_substring_start_location,
          tolower((int)(*index_of_case_ignored)));

      if (NULL == pointer_to_first_char_in_substring) {
        pointer_to_first_char_in_substring =
            strchr(pointer_to_last_char +
                       num_of_chars_to_move_substring_start_location,
...
int sl_string__first_index_of(const sl_string_t string, const char *index_of) {
  ...
  return ((NULL != pointer_to_first)
              ? (int)sl_utils__pointer_distance(pointer_to_first, string)
              : sl_string_error);
}

int sl_string__first_index_of_ignore_case(const sl_string_t string,
                                          const char *index_of_case_ignored) {
  int index_of_string = sl_string_error;
  ...
  if (NULL != index_of_case_ignored) {
    ...
    const size_t length_of_index_of_case_ignored =
        strlen(index_of_case_ignored);
    while (NULL != pointer_to_first_char_in_substring) {
      size_t num_of_chars_to_move_substring_start_location = 1;

      if (0 == strncasecmp(pointer_to_first_char_in_substring,
                           index_of_case_ignored,
                           length_of_index_of_case_ignored)) {
        index_of_string = (int)sl_utils__pointer_distance(
            pointer_to_first_char_in_substring, string);
        break;
      }

      const char *first_char_in_substring_storage_ptr =
          pointer_to_first_char_in_substring;
      pointer_to_first_char_in_substring =
          strchr(first_char_in_substring_storage_ptr +
                     num_of_chars_to_move_substring_start_location,
                 tolower((int)(*index_of_case_ignored)));
...
bool sl_string__contains_ignore_case(const sl_string_t string,
                                     const char *substring_case_ignored) {
  return (sl_string_error != sl_string__first_index_of_ignore_case(
                                 string, substring_case_ignored));
}
...
int sl_string__count_of(const sl_string_t string, const char *count_of) {
  ...
  if (NULL != count_of) {
    ...
    while (NULL != pointer_to_first_in_substring) {
      count++;
      pointer_to_first_in_substring = strstr(
          pointer_to_first_in_substring + length_of_count_of_string, count_of);
    }
  }
...
bool sl_string__begins_with(const sl_string_t string, const char *begin_with) {
  const size_t substring_length =
      (NULL == begin_with) ? 0U : strlen(begin_with);
  return sl_string__begins_with_limited_chars(string, begin_with,
                                              substring_length);
}
...
bool sl_string__begins_with_whole_word_ignore_case(
    const sl_string_t string, const char *begin_with_case_ignored,
    const char *delimiter_string) {
  bool result = false;
...
bool sl_string__ends_with(const sl_string_t string, const char *ends_with) {
  ...
  if (NULL != ends_with) {
    ...
    if (string_length >= substring_length) {
      if (0 == strncmp(string + string_length - substring_length, ends_with,
                       substring_length)) {
        result = true;
      }
    }
...
bool sl_string__ends_with_ignore_case(const sl_string_t string,
                                      const char *ends_with_case_ignored) {
  bool result = false;
  ...
  if (NULL != ends_with_case_ignored) {
    ...
    if (string_length >= substring_length) {
      if (0 == strncasecmp(string + string_length - substring_length,
                           ends_with_case_ignored, substring_length)) {
        result = true;
      }
    }
  }
...
bool sl_string__erase(sl_string_t string, const char *erase) {
  ...
  if (NULL != erase) {
    ...
    if (sl_string_error != index_to_erase) {
      result = sl_string__erase_after(string, (sl_string_size_t)index_to_erase,
                                      (sl_string_size_t)strlen(erase));
    }
  }
...
bool sl_string__erase_last(sl_string_t string, sl_string_size_t n_chars) {
  ...
  const sl_string_size_t start_of_erase_index =
      (n_chars > string_length) ? 0U
                                : (sl_string_size_t)(string_length - n_chars);
...
bool sl_string__replace_first(sl_string_t string, const char *replace,
                              const char *replace_with) {
  bool result = false;

  if ((NULL != replace) && (NULL != replace_with)) {
    const int start_of_replace_index =
        sl_string__first_index_of(string, replace);
    result = true;

    if (sl_string_error != start_of_replace_index) {
      (void)sl_string__erase_after(string,
                                   (sl_string_size_t)start_of_replace_index,
                                   (sl_string_size_t)strlen(replace));
      result = sl_string__insert_at(string, (uint16_t)start_of_replace_index,
                                    replace_with);
    }
  }
...
bool sl_string__replace_last(sl_string_t string, const char *replace,
                             const char *replace_with) {
  bool result = false;

  if ((NULL != replace) && (NULL != replace_with)) {
    const int start_of_replace_index =
        sl_string__last_index_of(string, replace);
    result = true;
    ...
    if (sl_string_error != start_of_replace_index) {
      (void)sl_string__erase_after(string,
                                   (sl_string_size_t)start_of_replace_index,
                                   (sl_string_size_t)strlen(replace));
      result = sl_string__insert_at(
          string, (sl_string_size_t)start_of_replace_index, replace_with);
    }
  }
...
int sl_string__replace_all(sl_string_t string, const char *replace,
                           const char *replace_with) {
  ...
  if ((NULL != replace) && (NULL != replace_with)) {
    ...
    const sl_string_size_t length_of_replace =
        (sl_string_size_t)strlen(replace);
    while (sl_string_error != start_of_replace_index) {
      bool result = sl_string__erase_after(
          string, (sl_string_size_t)start_of_replace_index, length_of_replace);
      if (result) {
